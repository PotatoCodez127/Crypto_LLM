...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using rolling mean and std for more sensitivity
    df['cvd_20_mean'] = df['cvd_20'].rolling(window=100).mean()
    df['cvd_20_std'] = df['cvd_20'].rolling(window=100).std()
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20_mean']) / (df['cvd_20_std'] + 1e-8)
    
    # Normalize z_score using rolling mean and std
    df['zscore_mean'] = df['z_score_50'].rolling(window=100).mean()
    df['zscore_std'] = df['z_score_50'].rolling(window=100).std()
    df['zscore_norm'] = (df['z_score_50'] - df['zscore_mean']) / (df['zscore_std'] + 1e-8)
    
    df['raw_signal'] = 0
    # Require stronger extremes and volatility moderately above median
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = (vol_ratio > 1.2) & (df['volatility_20'] > 0.001)  # volatility at least 20% above median and absolute threshold
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range, less adaptive to reduce stop distance
