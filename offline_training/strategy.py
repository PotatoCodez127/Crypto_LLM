...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD with shorter window for responsiveness
    ...
    long_condition = (df['cvd_z'] < -0.5) & (df['z_score_50'] < -0.8) & vol_filter
    short_condition = (df['cvd_z'] > 0.5) & (df['z_score_50'] > 0.8) & vol_filter
