...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Relax volatility requirement to capture more opportunities
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.0  # volatility at least above median
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range for better risk management
        ...
        if position == 0:
            ...
            # Trailing stop logic with profit taking at 1.5*ATR
            new_stop = close - atr_multiplier * atr
            # Ensure stop never moves below entry - 1.8*ATR (tighter max loss)
            max_loss_stop = entry_price - 1.8 * atr
            # Take profit if price moves 1.5*ATR in our favor
            profit_target = entry_price + 1.5 * atr
