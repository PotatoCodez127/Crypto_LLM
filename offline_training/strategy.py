...
def get_signals(df):
    ...
    vol_strong = vol_ratio > 1.0  # volatility at least at median
    long_condition = (df['cvd_robust'] < -1.2) & (df['zscore_norm'] < -0.8) & vol_strong
    short_condition = (df['cvd_robust'] > 1.2) & (df['zscore_norm'] > 0.8) & vol_strong
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 1.2 and 3.0
            atr_multiplier = 1.2 + (1.8 / (1.0 + np.exp(-vol_ratio_local + -1.0)))
