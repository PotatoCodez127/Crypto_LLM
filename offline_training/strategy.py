...
def get_signals(df):
    ...
    # Additional price position filter: deviation from short-term SMA
    df['sma_20'] = df['close'].rolling(window=20).mean()
    df['price_vs_sma'] = (df['close'] - df['sma_20']) / (df['close'].rolling(window=20).std() + 1e-8)
    
    df['raw_signal'] = 0
    # Require stronger extremes and moderate volatility (not too high)
    vol_moderate = df['volatility_20'] < (1.5 * df['vol_median'])
    long_condition = (df['cvd_robust'] < -2.0) & (df['z_score_50'] < -1.5) & vol_moderate & (df['price_vs_sma'] < -0.5)
    short_condition = (df['cvd_robust'] > 2.0) & (df['z_score_50'] > 1.5) & vol_moderate & (df['price_vs_sma'] > 0.5)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower adaptive range for tighter stops
            vol_ratio = vol / vol_med
            # Sigmoid scaling between 2.0 and 3.0
            atr_multiplier = 2.0 + (1.0 / (1.0 + np.exp(-vol_ratio + 1.0)))
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 2.0*ATR (tighter max loss)
