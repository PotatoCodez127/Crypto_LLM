...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using robust rolling percentiles (IQR) - shorter window for adaptability
    window_iqr = 80
    ...
    df['raw_signal'] = 0
    # Require extremes and volatility moderately above median
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.15  # volatility at least 15% above median
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range, more responsive to volatility regimes
            vol_ratio_local = vol / vol_med
            # Sigmoid scaling between 2.0 and 3.0
            atr_multiplier = 2.0 + (1.0 / (1.0 + np.exp(-vol_ratio_local + 1.0)))
