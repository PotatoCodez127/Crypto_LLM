...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require moderate extremes and volatility above median but not extreme
    vol_above_median = df['volatility_20'] > df['vol_median']
    # Limit volatility to less than 3x median to avoid extreme regimes
    vol_not_extreme = df['volatility_20'] < (df['vol_median'] * 3.0)
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above_median & vol_not_extreme
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above_median & vol_not_extreme
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Adaptive to volatility regimes but less aggressive
            vol_ratio = vol / vol_med
            # Use linear scaling between 1.2 and 3.0
            # When vol_ratio = 1 (vol at median) => multiplier ~ 1.8
            # When vol_ratio = 3 (high) => multiplier ~ 2.6
            atr_multiplier = 1.2 + (1.8 * (vol_ratio - 1.0) / (3.0 - 1.0))
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 1.2*ATR (max loss protection)
