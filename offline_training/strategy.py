...
def get_signals(df):
    ...
    # Price position within recent range (20-bar)
    ...
    df['raw_signal'] = 0
    # Slightly relaxed volatility condition, stricter CVD/Z thresholds
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (more signals)
    # Require price position confirmation: long when price near bottom (bottom 30%), short near top (top 30%)
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -2.0) & vol_strong & (df['price_pos'] < 0.3)
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 2.0) & vol_strong & (df['price_pos'] > 0.7)

    cooldown = 10  # reduced cooldown for more frequent opportunities
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More responsive scaling based on volatility regime
            vol_ratio_local = vol / vol_med
            # Exponential scaling: higher volatility -> larger multiplier (range 2.0 to 4.0)
            atr_multiplier = 2.0 + 2.0 * (1.0 - np.exp(-vol_ratio_local))
