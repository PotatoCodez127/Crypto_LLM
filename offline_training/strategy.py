...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Adjusted thresholds for better sensitivity while maintaining edge
    vol_above_median = df['volatility_20'] > (0.8 * df['vol_median'])
    long_condition = (df['cvd_robust'] < -1.3) & (df['z_score_50'] < -1.0) & vol_above_median
    short_condition = (df['cvd_robust'] > 1.3) & (df['z_score_50'] > 1.0) & vol_above_median
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More responsive multiplier to volatility changes
            vol_ratio = vol / vol_med
            # Adjusted sigmoid to produce multiplier between 1.2 and 3.5
