...
def get_signals(df):
    ...
    vol_above_median = df['volatility_20'] > (1.3 * df['vol_median'])
    # Adaptive percentile thresholds for CVD
    df['cvd_20_q10'] = df['cvd_20'].rolling(window=200).quantile(0.10)
    df['cvd_20_q90'] = df['cvd_20'].rolling(window=200).quantile(0.90)
    cvd_extreme_long = df['cvd_20'] < df['cvd_20_q10']
    cvd_extreme_short = df['cvd_20'] > df['cvd_20_q90']
    
    long_condition = (df['cvd_robust'] < -2.0) & (df['z_score_50'] < -1.5) & vol_above_median & cvd_extreme_long
    short_condition = (df['cvd_robust'] > 2.0) & (df['z_score_50'] > 1.5) & vol_above_median & cvd_extreme_short
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter adaptive range for volatility regimes
            vol_ratio = vol / vol_med
            # Sigmoid scaling to keep multiplier between 1.2 and 2.5
            atr_multiplier = 1.2 + (1.3 / (1.0 + np.exp(-vol_ratio + 1.2)))
