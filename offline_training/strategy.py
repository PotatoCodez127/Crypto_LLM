...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Slightly relaxed extremes and volatility condition
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (was 30%)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range to reduce risk
            vol_ratio_local = vol / vol_med
            # Use linear scaling to keep multiplier between 1.5 and 2.5
            atr_multiplier = 1.5 + (1.0 * (vol_ratio_local - 1.0))
