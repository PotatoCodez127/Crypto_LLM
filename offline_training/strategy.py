...
def get_signals(df):
    ...
    # Additional percentile filter for extreme CVD raw values
    df['cvd_20_q10'] = df['cvd_20'].rolling(window=100).quantile(0.10)
    df['cvd_20_q90'] = df['cvd_20'].rolling(window=100).quantile(0.90)
    ...
    vol_strong = vol_ratio > 1.5  # volatility at least 50% above median (more selective)
    # More extreme thresholds for CVD robust and Z-score
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -2.0) & vol_strong & (df['cvd_20'] < df['cvd_20_q10'])
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 2.0) & vol_strong & (df['cvd_20'] > df['cvd_20_q90'])
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 2.0 and 4.0
            atr_multiplier = 2.0 + (2.0 / (1.0 + np.exp(-vol_ratio_local + 1.0)))
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 2.5*ATR (max loss protection)
