...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Adjusted thresholds for more signals, volatility condition relaxed
    vol_above_median = df['volatility_20'] > (0.8 * df['vol_median'])
    # Add momentum filter: recent returns support direction
    df['ret_3'] = df['log_ret'].rolling(window=3).sum()
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above_median & (df['ret_3'] < 0)
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above_median & (df['ret_3'] > 0)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Sigmoid shifted to give multiplier between 1.2 and 3.0
            atr_multiplier = 1.2 + (1.8 / (1.0 + np.exp(-vol_ratio + 0.5)))
