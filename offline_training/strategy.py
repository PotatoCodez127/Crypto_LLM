...
def get_signals(df):
    ...
    # Additional momentum filter: SMA 20
    df['sma_20'] = df['close'].rolling(window=20).mean()
    
    df['raw_signal'] = 0
    # Relaxed thresholds and volatility condition
    vol_above = df['volatility_20'] > (0.8 * df['vol_median'])
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_above & (df['close'] > df['sma_20'])
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_above & (df['close'] < df['sma_20'])
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Adjusted range for tighter stops in low volatility, wider in high
            vol_ratio = vol / vol_med
            # Sigmoid scaling between 1.2 and 3.5
            atr_multiplier = 1.2 + (2.3 / (1.0 + np.exp(-vol_ratio + 1.0)))
