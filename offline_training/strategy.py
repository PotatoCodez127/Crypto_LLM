...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using robust rolling percentiles (IQR) - faster adaptation
    ...
    df['raw_signal'] = 0
    # Slightly relaxed thresholds to capture more opportunities while maintaining edge
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.2  # volatility at least 20% above median (was 1.3)
    long_condition = (df['cvd_robust'] < -1.8) & (df['zscore_norm'] < -1.3) & vol_strong
    short_condition = (df['cvd_robust'] > 1.8) & (df['zscore_norm'] > 1.3) & vol_strong
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range for more consistent stop placement
            vol_ratio_local = vol / vol_med
            # Linear scaling between 2.0 and 3.0 based on volatility ratio
            atr_multiplier = 2.0 + (vol_ratio_local - 1.0) * 1.0
