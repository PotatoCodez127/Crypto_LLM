...
def get_signals(df):
    ...
    long_condition = (df['cvd_robust'] < -1.8) & (df['zscore_norm'] < -2.3) & vol_strong
    short_condition = (df['cvd_robust'] > 1.8) & (df['zscore_norm'] > 2.3) & vol_strong
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 2.0 and III.0
            atr_multiplier = 2.0 + (1.0 / (1.0 + np.exp(-(vol_ratio_local - 0.5)*2)))
