...
def get_signals(df):
    ...
    cvd_window = 50
    df['cvd_20_q25'] = df['cvd_20'].rolling(window=cvd_window).quantile(0.25)
    df['cvd_20_q75'] = df['cvd_20'].rolling(window=cvd_window).quantile(0.75)
    df['cvd_iqr'] = df['cvd_20_q75'] - df['cvd_20_q25']
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20'].rolling(window=cvd_window).median()) / (df['cvd_iqr'] + 1e-8)
    
    # Normalize z_score using rolling median and IQR to adapt to regime
    z_window = 50
    df['zscore_median'] = df['z_score_50'].rolling(window=z_window).median()
    df['zscore_q25'] = df['z_score_50'].rolling(window=z_window).quantile(0.25)
    df['zscore_q75'] = df['z_score_50'].rolling(window=z_window).quantile(0.75)
    df['zscore_iqr'] = df['zscore_q75'] - df['zscore_q25']
    df['zscore_norm'] = (df['z_score_50'] - df['zscore_median']) / (df['zscore_iqr'] + 1e-8)
    
    df['raw_signal'] = 0
    # Require moderate extremes and volatility above median
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.2  # volatility at least 20% above median
    long_condition = (df['cvd_robust'] < -1.5) & (df['zscore_norm'] < -1.2) & vol_strong
    short_condition = (df['cvd_robust'] > 1.5) & (df['zscore_norm'] > 1.2) & vol_strong
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range, less extreme stops
