...
def get_signals(df):
    ...
    # Add trend alignment: long only when price above SMA200, short only when below
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -1.8) & vol_strong & (df['close'] > df['sma_200'])
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 1.8) & vol_strong & (df['close'] < df['sma_200'])
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0 and atr > 0:
            # Tighter range, more conservative stops
            ...
            # Minimum ATR requirement to avoid noise
            if atr < df['atr'].rolling(window=50).median().iloc[i] * 0.3:
                atr_multiplier = atr_multiplier * 1.5  # widen slightly if ATR too small
        else:
            atr_multiplier = 2.0
