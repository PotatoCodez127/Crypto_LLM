...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Adjusted thresholds for better trade frequency and quality
    # Use 80% of median volatility to allow more trades while still filtering low-vol periods
    vol_above_threshold = df['volatility_20'] > (0.8 * df['vol_median'])
    # Moderate thresholds to capture more opportunities
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_above_threshold
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_above_threshold

    cooldown = 8  # Reduced cooldown for higher frequency
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range for more consistent stops
        ...
        if position == 0:
            ...
            # Slightly relaxed max loss protection to allow more room for volatility
