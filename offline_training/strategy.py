...
def get_signals(df):
    ...
    # Allow signals when volatility is not extremely low (above 0.7 * median)
    vol_condition = df['volatility_20'] > (0.7 * df['vol_median'])
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Less aggressive scaling to reduce stop distance
            vol_ratio = vol / vol_med
            # Sigmoid scaling adjusted to range 1.2 to 3.0
