...
def get_signals(df):
    ...
    # Volume confirmation: require volume above its 20-period moving average
    df['volume_ma20'] = df['volume'].rolling(window=20).mean()
    df['volume_above_ma'] = df['volume'] > df['volume_ma20']
    
    df['raw_signal'] = 0
    # Slightly relaxed thresholds for more signals, plus volume confirmation
    vol_above_median = df['volatility_20'] > df['vol_median']
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above_median & df['volume_above_ma']
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above_median & df['volume_above_ma']
    ...
    for i in range(len(df)):
        ...
        if position == 0:
            if raw == 1:
                ...
                take_profit = close + 2.0 * atr  # 2*ATR take profit
