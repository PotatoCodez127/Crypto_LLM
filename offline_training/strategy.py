...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Slightly relaxed thresholds to capture more opportunities
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (was 1.3)
    ...
    # Add confirmation: require condition to hold for 2 consecutive bars
    long_confirmed = long_condition & long_condition.shift(1).fillna(False)
    short_confirmed = short_condition & short_condition.shift(1).fillna(False)

    cooldown = 10  # reduced from 20 to allow more frequent signals
    last_signal_idx = -cooldown
    for i in range(len(df)):
        if i < last_signal_idx + cooldown:
            continue
        if long_confirmed.iloc[i]:
            df.iloc[i, df.columns.get_loc('raw_signal')] = 1
            last_signal_idx = i
        elif short_confirmed.iloc[i]:
            df.iloc[i, df.columns.get_loc('raw_signal')] = -1
            last_signal_idx = i
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range to avoid excessive stop distance
            vol_ratio_local = vol / vol_med
            # Sigmoid scaling between 1.5 and 2.5 (was 1.8-3.5)
            atr_multiplier = 1.5 + (1.0 / (1.0 + np.exp(-vol_ratio_local + 1.0)))
