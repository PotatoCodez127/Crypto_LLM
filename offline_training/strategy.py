...
def get_signals(df):
    ...
    # Revised Hypothesis: Follow buying pressure when price is moderately undervalued,
    # and follow selling pressure when price is moderately overvalued.
    # Buy when cumulative volume delta positive (buying pressure) and price below normal (Z-score < -1.5)
    long_condition = (df['cvd_20'] > 0) & (df['z_score_50'] < -1.5)
    
    # Sell when cumulative volume delta negative (selling pressure) and price above normal (Z-score > 1.5)
