...
def get_signals(df):
    ...
    vol_strong = vol_ratio > 1.2  # volatility at least 20% above median (more opportunities)
    # Stricter thresholds for CVD and Z-score
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -2.0) & vol_strong & (df['close'] > df['sma_200'])
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 2.0) & vol_strong & (df['close'] < df['sma_200'])
