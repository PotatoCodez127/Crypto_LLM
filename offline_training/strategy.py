...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Volume confirmation: require volume above its 20-period average
    df['volume_ma20'] = df['volume'].rolling(window=20).mean()
    volume_strong = df['volume'] > df['volume_ma20']
    # Adjusted volatility filter: require volatility above 0.8 * median (less strict)
    vol_above_threshold = df['volatility_20'] > (0.8 * df['vol_median'])
    # Relax CVD thresholds, tighten Z-score thresholds
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above_threshold & volume_strong
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above_threshold & volume_strong

    cooldown = 8  # Reduced from 12 to allow more frequent trades
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Linear scaling between 1.2 and 3.0 based on volatility ratio
            vol_ratio = vol / vol_med
            # Clamp vol_ratio between 0.5 and 2.0 for stable multiplier
            clamped_ratio = max(0.5, min(2.0, vol_ratio))
            atr_multiplier = 1.2 + (clamped_ratio - 0.5) * (3.0 - 1.2) / (2.0 - 0.5)
