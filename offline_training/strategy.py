...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using robust rolling percentiles (IQR) with larger window for stability
    window_iqr = 120
    df['cvd_20_q25'] = df['cvd_20'].rolling(window=window_iqr).quantile(0.25)
    df['cvd_20_q75'] = df['cvd_20'].rolling(window=window_iqr).quantile(0.75)
    df['cvd_iqr'] = df['cvd_20_q75'] - df['cvd_20_q25']
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20'].rolling(window=window_iqr).median()) / (df['cvd_iqr'] + 1e-8)
    
    # Normalize z_score using rolling median and IQR to adapt to regime
    df['zscore_median'] = df['z_score_50'].rolling(window=window_iqr).median()
    df['zscore_q25'] = df['z_score_50'].rolling(window=window_iqr).quantile(0.25)
    df['zscore_q75'] = df['z_score_50'].rolling(window=window_iqr).quantile(0.75)
    df['zscore_iqr'] = df['zscore_q75'] - df['zscore_q25']
    df['zscore_norm'] = (df['z_score_50'] - df['zscore_median']) / (df['zscore_iqr'] + 1e-8)
    
    df['raw_signal'] = 0
    # Require stronger extremes and volatility moderately above median
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.25  # volatility at least 25% above median
    # More selective thresholds to improve signal quality
    long_condition = (df['cvd_robust'] < -2.3) & (df['zscore_norm'] < -1.8) & vol_strong
    short_condition = (df['cvd_robust'] > 2.3) & (df['zscore_norm'] > 1.8) & vol_strong
