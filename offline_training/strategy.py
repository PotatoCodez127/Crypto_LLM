...
def get_signals(df):
    ...
    # More selective: volatility between 1.2x and 2.5x median to avoid extreme noise
    vol_strong = (vol_ratio > 1.2) & (vol_ratio < 2.5)
    # Stricter thresholds for CVD and Z-score
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More conservative scaling: narrower range, less sensitive to volatility spikes
            vol_ratio_local = vol / vol_med
            # Keep multiplier between 2.0 and 3.0, using linear interpolation within bounds
            if vol_ratio_local <= 1.0:
                atr_multiplier = 2.0 + (vol_ratio_local - 0.5) * 0.4  # gentle increase
            else:
                atr_multiplier = 2.4 + (vol_ratio_local - 1.0) * 0.6
