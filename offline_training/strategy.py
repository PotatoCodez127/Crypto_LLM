...
def get_signals(df):
    ...
    vol_filter = df['volatility_20'] > (df['vol_median'] * 0.3)
    
    long_condition = (df['cvd_z'] < -1.0) & (df['z_score_50'] < -1.0) & vol_filter
    short_condition = (df['cvd_z'] > 1.0) & (df['z_score_50'] > 1.0) & vol_filter
