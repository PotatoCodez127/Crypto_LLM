...
def get_signals(df):
    ...
    - Long when MACD histogram > 0.0003, RSI > 50, close > SMA20, and SMA20 > SMA50
    - Short when MACD histogram < -0.0003, RSI < 50, close < SMA20, and SMA20 < SMA50
    - Stop-loss with dynamic ATR multiplier (1.2-2.5) based on recent volatility.
    No cooldown period, tighter stops to cut losses.
    """
    # 1. Calculate MACD
    ...
    # Symmetric RSI thresholds around 50
    ...
    for i in range(len(df)):
        ...
        # Dynamic ATR multiplier based on volatility - tighter stops to cut losses
        if vol_med > 0:
