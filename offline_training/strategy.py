...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using rolling z-score (more sensitive)
    cvd_mean = df['cvd_20'].rolling(window=100).mean()
    cvd_std = df['cvd_20'].rolling(window=100).std()
    df['cvd_robust'] = (df['cvd_20'] - cvd_mean) / (cvd_std + 1e-8)
    
    df['raw_signal'] = 0
    # Adjusted thresholds for more signals, volatility within reasonable range
    vol_above_median = df['volatility_20'] > df['vol_median']
    vol_not_extreme = df['volatility_20'] < 2.0 * df['vol_median']
    long_condition = (df['cvd_robust'] < -1.0) & (df['z_score_50'] < -0.8) & vol_above_median & vol_not_extreme
    short_condition = (df['cvd_robust'] > 1.0) & (df['z_score_50'] > 0.8) & vol_above_median & vol_not_extreme
