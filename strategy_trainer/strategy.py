...
def get_signals(df):
    ...
    # Stricter thresholds
    long_condition = (df['cvd_robust'] < -1.5) & (df['zscore_norm'] < -2.0) & vol_strong & price_confirm_long
    short_condition = (df['cvd_robust'] > 1.5) & (df['zscore_norm'] > 2.0) & vol_strong & price_confirm_short
