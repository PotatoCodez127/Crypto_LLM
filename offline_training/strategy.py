...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Relax thresholds and remove volatility filter to increase signal frequency
    # Keep volatility filter optional: use it only if volatility is not extremely low
    vol_not_extreme_low = df['volatility_20'] > df['vol_median'] * 0.5
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_not_extreme_low
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_not_extreme_low
    ...
    for i in range(len(df)):
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 1.2*ATR (max loss protection) - less strict
