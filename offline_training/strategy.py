...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require moderate extremes and volatility above half median (avoid low-vol noise)
    vol_above_half = df['volatility_20'] > (df['vol_median'] * 0.5)
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_above_half
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_above_half
