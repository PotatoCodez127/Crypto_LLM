...
def get_signals(df):
    ...
    # Also allow moderate volatility if extremes are very strong
    extreme_cvd = (df['cvd_robust'].abs() > 2.5)
    extreme_zscore = (df['zscore_norm'].abs() > 2.0)
    moderate_vol = (vol_ratio > 0.8) & (vol_ratio <= 1.2)
    strong_vol = vol_strong
    
    long_condition = (
        ((df['cvd_robust'] < -1.5) & (df['zscore_norm'] < -1.2) & strong_vol) |
        (extreme_cvd & (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -1.8) & moderate_vol)
    )
    short_condition = (
        ((df['cvd_robust'] > 1.5) & (df['zscore_norm'] > 1.2) & strong_vol) |
        (extreme_cvd & (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 1.8) & moderate_vol)
