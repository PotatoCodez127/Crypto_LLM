...
def get_signals(df):
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Adjusted range for tighter risk management
            vol_ratio = vol / vol_med
            # Sigmoid scaling between 1.8 and 3.5
            atr_multiplier = 1.8 + (1.7 / (1.0 + np.exp(-vol_ratio + 1.2)))
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 1.8*ATR (max loss protection)
