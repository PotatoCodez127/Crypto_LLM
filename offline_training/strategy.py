...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using robust rolling percentiles (IQR) with faster adaptation
    ...
    # Normalize z_score using rolling median and IQR with shorter window
    ...
    # Require CVD and Z-score to be in same direction, with moderate extremes
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More adaptive and wider range for ATR multiplier
            vol_ratio_local = vol / vol_med
            # Sigmoid shifted to allow multiplier between 1.5 and 4.0
