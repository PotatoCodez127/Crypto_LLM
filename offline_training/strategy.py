...
def get_signals(df):
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range, more conservative adaptation
            vol_ratio = vol / vol_med
            # Sigmoid scaling between 1.8 and 3.5, shifted to be less aggressive
            atr_multiplier = 1.8 + (1.7 / (1.0 + np.exp(-vol_ratio + 0.8)))
