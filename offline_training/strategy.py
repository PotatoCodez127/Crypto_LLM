...
def get_signals(df):
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More adaptive scaling: narrower range, more responsive
            vol_ratio_local = vol / vol_med
            # Linear scaling between 1.5 and 3.0 based on vol_ratio_local
            # When vol_ratio_local == 1 (median), multiplier = 2.0
            # When vol_ratio_local is low (<0.8) -> near 1.5, high (>1.5) -> near 3.0
            atr_multiplier = 1.5 + (vol_ratio_local - 0.8) * (1.5 / 0.7) if vol_ratio_local >= 0.8 else 1.5
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 2.5*ATR (allow slightly more room)
