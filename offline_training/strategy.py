...
def get_signals(df):
    ...
    # Trend filter: price relative to 50-period SMA
    df['sma_50'] = df['close'].rolling(window=50).mean()
    df['above_sma'] = df['close'] > df['sma_50']
    df['below_sma'] = df['close'] < df['sma_50']
    ...
    # Stricter thresholds for CVD and Z-score, plus trend alignment
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -2.0) & vol_strong & df['below_sma']
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 2.0) & vol_strong & df['above_sma']

    cooldown = 30  # increased to reduce overtrading
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range, less extreme adaptation
        ...
        if position == 0:
            ...
            # Ensure stop never moves below entry - 1.5*ATR (tighter max loss protection)
