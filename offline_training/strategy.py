...
def get_signals(df):
    ...
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_above_median
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_above_median
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Adaptive to volatility regimes, but less extreme
            vol_ratio = vol / vol_med
            # Sigmoid scaling to keep multiplier between 1.2 and 3.5
            atr_multiplier = 1.2 + (2.0 / (1.0 + np.exp(-vol_ratio + 1.0)))
