...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require extremes but allow signals in various volatility regimes
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0)
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0)
    ...
    for i in range(len(df)):
        ...
        if position == 0:
            ...
            # Allow slightly larger max loss to avoid being stopped out too early
