...
def get_signals(df):
    ...
    # Additional CVD momentum: rate of change over last 5 periods
    df['cvd_roc'] = df['cvd_20'].diff(5) / (df['cvd_20'].rolling(window=5).std() + 1e-8)
    
    df['raw_signal'] = 0
    # Relax thresholds to capture more opportunities
    vol_above_median = df['volatility_20'] > df['vol_median'] * 0.8  # Less strict
    long_condition = (df['cvd_robust'] < -1.0) & (df['z_score_50'] < -0.8) & vol_above_median & (df['cvd_roc'] < 0)
    short_condition = (df['cvd_robust'] > 1.0) & (df['z_score_50'] > 0.8) & vol_above_median & (df['cvd_roc'] > 0)

    cooldown = 8  # Reduced to allow more frequent trades
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Less aggressive scaling to reduce stop-loss tightness
            vol_ratio = vol / vol_med
            # Linear scaling between 1.2 and 3.0
        ...
        if position == 0:
            ...
            max_loss_stop = entry_price - 2.0 * atr  # Allow more room before stop
