...
def get_signals(df):
    ...
    # Add momentum filter: recent price change in signal direction
    df['momentum_5'] = df['close'].pct_change(5)
    
    df['raw_signal'] = 0
    # Slightly relaxed thresholds and volatility condition
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (more signals)
    long_condition = (df['cvd_robust'] < -1.5) & (df['zscore_norm'] < -1.0) & vol_strong & (df['momentum_5'] < 0)
    short_condition = (df['cvd_robust'] > 1.5) & (df['zscore_norm'] > 1.0) & vol_strong & (df['momentum_5'] > 0)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range for tighter stops
