...
def get_signals(df):
    ...
    # Normalize z_score using rolling median and MAD for robustness
    ...
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -1.2) & vol_strong
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 1.2) & vol_strong
