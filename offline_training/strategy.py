...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require stronger extremes and volatility within reasonable regime
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_condition = (vol_ratio > 0.5) & (vol_ratio < 2.0)
    long_condition = (df['cvd_robust'] < -2.0) & (df['z_score_50'] < -1.5) & vol_condition
    short_condition = (df['cvd_robust'] > 2.0) & (df['z_score_50'] > 1.5) & vol_condition
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # More conservative adaptive multiplier
            vol_ratio = vol / vol_med
            # Sigmoid-like scaling to keep multiplier between 2.0 and 3.5
            atr_multiplier = 2.0 + (1.5 / (1.0 + np.exp(-vol_ratio + 0.5)))
            atr_multiplier = max(2.0, min(3.5, atr_multiplier))
        else:
            atr_multiplier = 2.5
