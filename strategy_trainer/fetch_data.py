...
def fetch_historical_data():
    print("📡 Initializing connection to Binance via CCXT...")
    ...
    if os.path.exists(OUTPUT_FILE):
        ...
        print(
            f"🗓️ No existing data. Fetching {YEARS_TO_FETCH} years of {TIMEFRAME} data for {SYMBOL}..."
        )
    ...
    while since < end_time:
        try:
            ...
            readable_date = datetime.fromtimestamp(last_timestamp / 1000, tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M"
            )
            print(
                f"   [+] Fetched missing data up to: {readable_date} | New Candles: {len(all_candles)}"
            )
