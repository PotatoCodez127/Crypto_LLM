...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require stronger extremes and volatility above median but not extreme
    vol_above_median = df['volatility_20'] > df['vol_median']
    vol_not_extreme = df['volatility_20'] < 2.0 * df['vol_median']
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -0.8) & vol_above_median & vol_not_extreme
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 0.8) & vol_above_median & vol_not_extreme
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Less aggressive scaling to avoid excessive stops
