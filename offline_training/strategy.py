...
def get_signals(df):
    ...
    df['vol_median'] = df['volatility'].rolling(window=50).median().bfill().fillna(df['volatility'])

    # 2. Generate raw signals with revised conditions
    df['raw_signal'] = 0
    # No volume filter
    volume_long = True
    volume_short = True
    # Increased MACD threshold to filter out weak signals
    macd_threshold = 0.001
    # More extreme RSI thresholds to capture stronger momentum
    ...
    # No cooldown period to allow immediate re-entry
    cooldown = 0
    last_signal_idx = -1
