...
def engineer_features():
    ...
    df["close_zscore_50"] = (df["close"] - df["close"].rolling(50).mean()) / df["close"].rolling(
        50
    ).std()
    df["volume_zscore_24"] = (df["volume"] - df["volume"].rolling(24).mean()) / df[
        "volume"
    ].rolling(24).std()
    ...
    print("📊 New Features Available: cvd_trend, atr_14, close_zscore_50, volume_zscore_24")
