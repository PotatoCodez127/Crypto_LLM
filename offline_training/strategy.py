...
def get_signals(df):
    ...
    # --- EXECUTION LOGIC ---
    # Normalized CVD using robust rolling percentiles (IQR) with longer window for stability
    df['cvd_20_q25'] = df['cvd_20'].rolling(window=200).quantile(0.25)
    df['cvd_20_q75'] = df['cvd_20'].rolling(window=200).quantile(0.75)
    df['cvd_iqr'] = df['cvd_20_q75'] - df['cvd_20_q25']
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20'].rolling(window=200).median()) / (df['cvd_iqr'] + 1e-8)
    
    # Normalize z_score using rolling median and IQR to adapt to regime
    df['zscore_median'] = df['z_score_50'].rolling(window=200).median()
    df['zscore_q25'] = df['z_score_50'].rolling(window=200).quantile(0.25)
    df['zscore_q75'] = df['z_score_50'].rolling(window=200).quantile(0.75)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            atr_multiplier = 1.8 + (1.7 / (1.0 + np.exp(-vol_ratio_local + -0.5)))
