...
def get_signals(df):
    ...
    vol_strong = vol_ratio > 1.5  # Increased from 1.3 to 1.5: require 50% above median volatility
    long_condition = (df['cvd_robust'] < -2.5) & (df['zscore_norm'] < -2.0) & vol_strong  # Stricter thresholds
    short_condition = (df['cvd_robust'] > 2.5) & (df['zscore_norm'] > 2.0) & vol_strong

    cooldown = 30  # Increased from 20 to reduce overtrading
