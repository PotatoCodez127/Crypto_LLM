...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require volatility above median but not extreme to avoid whipsaw
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = (vol_ratio > 1.0) & (vol_ratio < 2.0)  # volatility above median but not extreme
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range, adaptive to volatility regimes
            vol_ratio_local = vol / vol_med
            # Use sigmoid-like scaling to keep multiplier between 1.5 and 2.5
