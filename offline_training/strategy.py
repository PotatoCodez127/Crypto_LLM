...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Volatility filter: require volatility above 80% of median (less restrictive)
    vol_above_threshold = df['volatility_20'] > (0.8 * df['vol_median'])
    # Momentum confirmation: recent 5-period return in same direction
    df['ret_5'] = df['close'].pct_change(5)
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above_threshold & (df['ret_5'] < 0)
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above_threshold & (df['ret_5'] > 0)
    ...
    for i in range(len(df)):
        ...
        if position == 0:
            ...
            # Allow slightly larger max loss to avoid being stopped too early
