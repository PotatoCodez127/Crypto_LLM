...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Slightly relaxed thresholds and volatility filter
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (less restrictive)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Narrower range for tighter stops, more responsive to volatility
            vol_ratio_local = vol / vol_med
            # Linear scaling between 1.5 and 2.5 based on vol_ratio_local
            # When vol_ratio_local == 1.0 (median), multiplier = 2.0
            atr_multiplier = 2.0 + (vol_ratio_local - 1.0) * 0.5
        ...
        if position == 0:
            ...
            max_loss_stop = entry_price - 1.5 * atr  # Tighter max loss
