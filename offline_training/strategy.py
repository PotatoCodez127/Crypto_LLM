...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Slightly relaxed thresholds and less strict volatility filter
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 1.2 and 5.0
            atr_multiplier = 1.2 + (3.8 / (1.0 + np.exp(-vol_ratio + 1.0)))
            atr_multiplier = max(1.2, min(5.0, atr_multiplier))
        else:
            atr_multiplier = 2.5
        ...
        if position == 0:
            ...
            # Take profit if we have 2*ATR profit
            take_profit_price = entry_price + 2.0 * atr
