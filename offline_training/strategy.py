...
def get_signals(df):
    ...
    # Use shorter window for CVD to be more responsive
    df['cvd_15'] = df['vol_delta'].rolling(window=15).sum()
    df['cvd_15_q25'] = df['cvd_15'].rolling(window=100).quantile(0.25)
    df['cvd_15_q75'] = df['cvd_15'].rolling(window=100).quantile(0.75)
    df['cvd_iqr'] = df['cvd_15_q75'] - df['cvd_15_q25']
    df['cvd_robust'] = (df['cvd_15'] - df['cvd_15'].rolling(window=100).median()) / (df['cvd_iqr'] + 1e-8)
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter adaptive range to reduce stop distance
