...
def get_signals(df):
    ...
    window_norm = 80
    df['cvd_20_q25'] = df['cvd_20'].rolling(window=window_norm).quantile(0.25)
    df['cvd_20_q75'] = df['cvd_20'].rolling(window=window_norm).quantile(0.75)
    df['cvd_iqr'] = df['cvd_20_q75'] - df['cvd_20_q25']
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20'].rolling(window=window_norm).median()) / (df['cvd_iqr'] + 1e-8)
    
    # Normalize z_score using rolling median and IQR to adapt to regime
    df['zscore_median'] = df['z_score_50'].rolling(window=window_norm).median()
    df['zscore_q25'] = df['z_score_50'].rolling(window=window_norm).quantile(0.25)
    df['zscore_q75'] = df['z_score_50'].rolling(window=window_norm).quantile(0.75)
    ...
    long_condition = (df['cvd_robust'] < -2.2) & (df['zscore_norm'] < -1.6) & vol_strong
    short_condition = (df['cvd_robust'] > 2.2) & (df['zscore_norm'] > 1.6) & vol_strong
