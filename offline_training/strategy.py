...
def get_signals(df):
    ...
    # Volatility filter: only trade when volatility is not too low
    df['vol_filter'] = df['volatility_20'] > (df['vol_median'] * 0.5)
    
    df['raw_signal'] = 0
    long_condition = (df['cvd_z'] < -1.5) & (df['z_score_50'] < -1.2) & df['vol_filter']
    short_condition = (df['cvd_z'] > 1.5) & (df['z_score_50'] > 1.2) & df['vol_filter']
