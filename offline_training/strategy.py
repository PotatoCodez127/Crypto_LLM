...
def get_signals(df):
    ...
    # Volume confirmation: require volume above median * 1.2
    ...
    vol_strong = vol_ratio > 1.1  # volatility at least 10% above median (relaxed)
    long_condition = (df['cvd_robust'] < -1.5) & (df['zscore_norm'] < -1.0) & vol_strong & df['volume_strong']
    short_condition = (df['cvd_robust'] > 1.5) & (df['zscore_norm'] > 1.0) & vol_strong & df['volume_strong']
