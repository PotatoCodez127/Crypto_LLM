...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require moderate extremes and volatility above 80% of median (more permissive)
    vol_threshold = df['vol_median'] * 0.8
    vol_above = df['volatility_20'] > vol_threshold
    long_condition = (df['cvd_robust'] < -1.2) & (df['z_score_50'] < -1.0) & vol_above
    short_condition = (df['cvd_robust'] > 1.2) & (df['z_score_50'] > 1.0) & vol_above
