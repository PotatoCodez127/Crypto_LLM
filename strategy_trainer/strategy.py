...
def get_signals(df):
    ...
    df['cvd_20_q25'] = df['cvd_20'].rolling(window=120).quantile(0.25)
    df['cvd_20_q75'] = df['cvd_20'].rolling(window=120).quantile(0.75)
    df['cvd_iqr'] = df['cvd_20_q75'] - df['cvd_20_q25']
    df['cvd_robust'] = (df['cvd_20'] - df['cvd_20'].rolling(window=120).median()) / (df['cvd_iqr'] + 1e-8)
    
    # Normalize z_score using rolling median and IQR to adapt to regime
    df['zscore_median'] = df['z_score_50'].rolling(window=120).median()
    df['zscore_q25'] = df['z_score_50'].rolling(window=120).quantile(0.25)
    df['zscore_q75'] = df['z_score_50'].rolling(window=120).quantile(0.75)
    ...
    long_condition = (df['cvd_robust'] < -2.0) & (df['zscore_norm'] < -2.5) & vol_strong
    short_condition = (df['cvd_robust'] > 2.0) & (df['zscore_norm'] > 2.5) & vol_strong
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 1.6 and 2.8
            atr_multiplier = 1.6 + (1.2 / (1.0 + np.exp(-vol_ratio_local + 0.0)))
            atr_multiplier = max(1.6, min(2.8, atr_multiplier))
        else:
            atr_multiplier = 2.2
