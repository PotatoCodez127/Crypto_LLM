...
def get_signals(df):
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            ...
            # Use sigmoid-like scaling to keep multiplier between 1.5 and 2.8
            atr_multiplier = 1.5 + (1.3 / (1.0 + np.exp(-vol_ratio_local + 1.0)))
            atr_multiplier = max(1.5, min(2.8, atr_multiplier))
        else:
            atr_multiplier = 2.0
