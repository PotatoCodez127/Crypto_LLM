...
def get_signals(df):
    ...
    df['raw_signal'] = 0
    # Require moderate extremes and volatility not too low
    vol_ratio = df['volatility_20'] / (df['vol_median'] + 1e-8)
    vol_ok = vol_ratio > 0.8  # allow slightly below median
    long_condition = (df['cvd_robust'] < -1.0) & (df['z_score_50'] < -0.8) & vol_ok
    short_condition = (df['cvd_robust'] > 1.0) & (df['z_score_50'] > 0.8) & vol_ok
    ...
    for i in range(len(df)):
        ...
        if vol_med > 0:
            # Tighter range, less aggressive adaptation
        ...
        if position == 0:
            ...
            # Trailing stop logic with profit taking at 2*ATR
            profit_target = entry_price + 2.0 * atr
